...
namespace :outbox_relay do
  ...
  desc 'Backfill commit_seq on historical rows (SB-2140). Args: batch_size (default 10000), sleep_ms (default 0)'
  task :backfill_commit_seq, %i[batch_size sleep_ms] => :environment do |_t, args|
    require 'outbox_relay'

    batch_size = (args[:batch_size] || 10_000).to_i
    sleep_seconds = (args[:sleep_ms] || 0).to_f / 1000.0

    if batch_size < 1
      puts "✗ ERROR: batch_size must be >= 1, got #{args[:batch_size].inspect}"
      exit 1
    end

    puts 'OutboxRelay Backfill commit_seq'
    puts '=' * 80
    puts "Batch size: #{batch_size} | sleep between batches: #{(sleep_seconds * 1000).to_i}ms"
    puts ''
    puts 'Sets commit_seq = sequence for rows the trigger has not assigned (historical'
    puts 'rows from before add_commit_seq). Idempotent and resumable — only touches'
    puts 'commit_seq IS NULL, batched by primary key. Safe to run while the trigger is'
    puts 'live and while consumers run; run before the Stage 2 consumer flip.'
    puts ''

    pending = OutboxRelay::OutboxEvent.where(commit_seq: nil)

    begin
      min_id = pending.minimum(:id)
      max_id = pending.maximum(:id)
    rescue StandardError => e
      puts "✗ ERROR: failed to scan for unassigned rows: #{e.message} (#{e.class.name})"
      OutboxRelay::Instrumentation::Tasks.task_error(
        e, task_name: 'outbox_relay:backfill_commit_seq', phase: 'scan', severity: 'high'
      )
      exit 1
    end

    if min_id.nil?
      puts '✓ Nothing to backfill — every row already has a commit_seq.'
      next
    end
    ...
    begin
      while lo <= max_id
        hi = lo + batch_size - 1
        # update_all with a SQL fragment: SET commit_seq = sequence. Bounded by the
        # PK range so each batch is a short, index-driven statement.
        updated = OutboxRelay::OutboxEvent
                  .where(id: lo..hi, commit_seq: nil)
                  .update_all('commit_seq = sequence')
      ...
      puts ''
      puts "✗ ERROR after #{total_updated} row(s): #{e.message} (#{e.class.name})"
      puts "  Re-run the task to resume — it picks up from the remaining NULLs."
      OutboxRelay.logger.error(
        event_name: 'rake_backfill_commit_seq_failed',
        rows_updated: total_updated, error: e.message, error_class: e.class.name,
        backtrace: e.backtrace&.first(10)&.join("\n")
      )
      OutboxRelay::Instrumentation::Tasks.task_error(
        e, task_name: 'outbox_relay:backfill_commit_seq', phase: 'update',
        rows_updated: total_updated, severity: 'critical'
      )
      exit 1
    end

    duration = Process.clock_gettime(Process::CLOCK_MONOTONIC) - started_at
    remaining = OutboxRelay::OutboxEvent.where(commit_seq: nil).count

    OutboxRelay.logger.info(
      event_name: 'rake_backfill_commit_seq_completed',
      rows_updated: total_updated, remaining_null: remaining,
      duration_ms: (duration * 1000).round(2)
    )
    ActiveSupport::Notifications.instrument(
      'outbox_relay.backfill_commit_seq.completed',
      rows_updated: total_updated, remaining_null: remaining, duration: duration
    )

    puts '=' * 80
    puts "✓ Updated #{total_updated} row(s) in #{format_duration(duration)}"
    if remaining.positive?
      # New rows committed during the run are assigned by the trigger, not NULL, so
      # a positive count here means rows were inserted without the trigger firing
      # (e.g. session_replication_role = replica) — investigate before Stage 2.
      puts "⚠ #{remaining} row(s) still have NULL commit_seq — re-run, and check that"
      puts '  the assignment trigger is enabled for all write paths before cutover.'
      exit 1
    else
      puts '✓ No rows with NULL commit_seq remain — safe to proceed to the Stage 2 cutover.'
    end
  end
