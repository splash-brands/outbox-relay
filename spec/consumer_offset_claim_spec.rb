...
RSpec.describe OutboxRelay::ConsumerOffset, 'partition claiming' do
  ...
  describe '#try_claim!' do
    context 'when partition is unclaimed' do
      ...
      it 'sets heartbeat_at to prove worker liveness' do
        freeze_time = Time.current
        allow(Time).to receive(:current).and_return(freeze_time)

        offset.try_claim!(consumer_instance_id: 'worker-1')

        expect(offset.reload.heartbeat_at).to be_within(1.second).of(freeze_time)
      end
    end
  ...
  describe '#renew_claim!' do
    context 'when holding the claim' do
      ...
      it 'updates heartbeat_at to prove worker liveness' do
        # Set heartbeat_at to an old value
        offset.update!(heartbeat_at: 10.minutes.ago)
        old_heartbeat = offset.heartbeat_at
        ...
        offset.reload
        expect(offset.heartbeat_at).to be_within(1.second).of(freeze_time)
        expect(offset.heartbeat_at).to be > old_heartbeat
      end

      it 'keeps heartbeat_at fresh even when no events are processed' do
        # This test verifies the fix for the false positive orphaned partition alerts.
        # When a partition has no new events, update_offset! is never called,
        # so heartbeat_at would get stale. renew_claim! must update heartbeat_at
        # to prevent PartitionMonitor from marking the partition as stale.
        offset.update!(heartbeat_at: 5.minutes.ago)

        # Simulate multiple heartbeats without any event processing
        3.times do
          offset.renew_claim!(consumer_instance_id: 'worker-1')
        end

        offset.reload
        # heartbeat_at should be recent, not 5 minutes ago
        expect(offset.heartbeat_at).to be > 1.minute.ago
      end
    end
