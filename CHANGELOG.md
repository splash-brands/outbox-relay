...
- **Consumer-group kill switch** — DB-flag enforcement that lets operators disable a consumer group without a deploy (SB-2198).
  - New model `OutboxRelay::ConsumerControl` reads the `outbox_relay_consumer_controls` table, keyed on the **base** `consumer_group` (no `_pN` partition suffix). Scope `disabled` = `where.not(disabled_at: nil)`; class helpers `.disabled_consumer_groups` (Set) and `.disabled?(consumer_group)`. Queries defensively rescue `ActiveRecord::StatementInvalid`, so the gem boots and runs normally before the host app provisions the table (kill switch simply inert). The table itself is owned/migrated by the host application (SB-2199).
